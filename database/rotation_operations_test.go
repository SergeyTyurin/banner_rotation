...
func setTestData(d databaseImpl) {
	...
	for i := 0; i < 10; i++ {
		d.CreateBanner(structures.Banner{Info: "banner_" + strconv.Itoa(i+1)})
	}
	for i := 0; i < 2; i++ {
		d.CreateGroup(structures.Group{Info: "group_" + strconv.Itoa(i+1)})
	}
	for i := 0; i < 4; i++ {
		d.CreateSlot(structures.Slot{Info: "slot_" + strconv.Itoa(i+1)})
	}
}
...
func TestAddToRotation(t *testing.T) {
	...
	t.Run("simple add", func(t *testing.T) {
		d.db.Exec(`TRUNCATE TABLE "Statistic" RESTART IDENTITY CASCADE`)
		slotId := 1
		bannerId := 2
...
func TestDeleteFromRotation(t *testing.T) {
	...
	t.Run("simple delete", func(t *testing.T) {
		...
		slotId := 1
		bannerId := 2
		d.AddToRotation(bannerId, slotId)
